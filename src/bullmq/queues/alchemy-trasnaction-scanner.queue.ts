...
@Injectable()
export class AlchemyTransactionScannerQueue implements OnModuleInit {
  private readonly logger = new Logger(AlchemyTransactionScannerQueue.name)

  constructor(
    @InjectQueue(ALCHEMY_TRANSACTION_SCANNER_QUEUE)
  ...
  private async addRecurringScanJob(): Promise<void> {
    try {
      await this.safelyUpsertScheduler(
        this.queue,
        ALCHEMY_TRANSACTION_SCANNER_QUEUE,
        { every: 10_000 },
        {
      ...
      this.logger.debug(`⏱️ Scheduler "${ALCHEMY_TRANSACTION_SCANNER_QUEUE}" upserted successfully`)
    } catch (error) {
