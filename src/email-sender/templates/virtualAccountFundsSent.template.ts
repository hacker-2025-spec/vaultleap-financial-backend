...
import type { VirtualAccountFundsSentEmailParams } from '../email-sender.service.types'

export const Subject = 'Vaultleap: A Payment Has Been Successfully Sent To Your Virtual Account'

export const FirstBlueBoxContent = ({ fullName, vaultName, amount, date, currency }: VirtualAccountFundsSentEmailParams) => `
            ${TEXT(`Hey ${fullName},`, '22px', 'margin-top: 0; margin-bottom: 4px')}

            ${TEXT(`Transaction Confirmation`, '18px', 'margin-top: 0; margin-bottom: 18px')}

            ${TEXT(`A payment has been successfully sent to your Virtual Account with name ${vaultName}`, '14px', 'margin-top: 0; margin-bottom: 14px;')}

            ${TEXT(`Transaction Details:`, '14px', 'margin-top: 0; margin-bottom: 0;')}
            ${TEXT(`Amount: ${amount} ${currency}`, '14px', 'margin-top: 0; margin-bottom: 0;')}
...
export const Body = (params: VirtualAccountFundsSentEmailParams) => `
        ${BLUE_BOX(FirstBlueBoxContent(params))}
        ${BLUE_BOX(SecondBlueBoxContent())}
        <tr>
          <td align="left">
            ${TEXT(`This email was intended for ${params.fullName}.`, '12px', 'margin-top: 0; margin-bottom: 0;')}
...
export const VirtualAccountFundsSentTemplate = (params: VirtualAccountFundsSentEmailParams) => ({
  Subject,
